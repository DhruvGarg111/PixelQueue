...
## 2024-05-10 - Memoize existing Konva canvas elements
**Learning:** In React-Konva applications, re-evaluating and reconciling the entire list of shapes on every frame during active drawing state changes (like `draftBox` updates) causes noticeable lag and framerate drops.
**Action:** Wrap the mapping of existing canvas elements in a `useMemo` hook (e.g., within `CanvasStage`) to prevent React and Konva from needlessly re-rendering the elements when their specific dependencies haven't changed.
